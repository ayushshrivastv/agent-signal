...
import { signalProgram, GameClient, GameType, Genre } from "../sdk/lib";
...
describe("signal", () => {
  ...
  const client = signalProgram.get(provider);
  ...
  it("Can update player username (name change) and verify success", async () => {
    const newUsername = "UpdatedPlayerName";
    const { transaction } = await client.updatePlayerAccount(
      user1.publicKey,
      newUsername,
      undefined
    );
    ...
    const playerAddress = client.utils.derivePlayerAddress(user1.publicKey)[0];
    const playerAccount = await client.fetchPlayerAccount(playerAddress);

    expect(playerAccount.username).to.equal(newUsername);
  });
