...
This Typescript sdk provides a convenient interface and methods for interacting with the on-chain signal program.

## Contents

- [signal SDK](#signal-sdk)
  - [Contents](#contents)
...
```typescript
import { signalProgram, GameType, Genre } from "@ayush/solana-sdk";

// Create a signal client using the '@solana/web3.js' active Connection and a defaultPayer
const client = signalProgram.getFromConnection(connection, defaultPayer); 
...
The `signalProgram` class gives client access to every instruction in the on-chain signal program.
...
```typescript
import { GameClient } from "@ayush/solana-sdk";
```

Get an instance representing an existing on-chain Game account:

```typescript
const signal = signalProgram.getFromConnection(connection, defaultPayer);
const gameClient = new GameClient(signal, address);
```
...
```typescript
const signal = signalProgram.getFromConnection(connection, defaultPayer);
const game = new GameClient.register(signal, ...);
```
...
```typescript
import { InstructionBuilder } from "@ayush/solana-sdk"
```
