...
import { IDL, type signal } from "./idl/signal";
...
export class signalProgram {
  ...
  public static getFromConnection(
    ...
    return new signalProgram(
      new SimpleProvider(connection, defaultPayer) as AnchorProvider,
      programId ?? PROGRAM_ID
    );
  }

  /// Static initializer for a signalProgram instance.
  public static get(
    provider: AnchorProvider,
    programId?: PublicKey
  ): signalProgram {
    return new signalProgram(provider, programId ?? PROGRAM_ID);
  }
  ...
  public async updateGameLeaderboard(
    ...
    game = game !== undefined ? this.getPublicKey(game) : undefined;
  ...
  public async registerPlayerEntryForLeaderBoard(
    ...
    const step = await this.builder.registerPlayerEntryStep(
      user,
      leaderboard,
      game
    );
